...
export default function AboutPage() {
  ...
  const t = { ...tr.common[lang], ...tr.about[lang] };
  ...
  const pStyle = { fontSize: '14px', color: '#444', lineHeight: 1.9, fontWeight: 300, margin: 0, whiteSpace: 'pre-line' };
      ...
      <div style={{ minHeight: '100vh', background: '#fafaf8', fontFamily: '"Noto Sans KR", sans-serif', color: '#1a1a1a' }}>
        <header style={{ padding: '36px 48px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid #f0f0f0' }}>
          ...
          <nav style={{ display: 'flex', gap: '32px', alignItems: 'center' }}>
            <a href="/how-it-works" style={{ fontSize: '11px', letterSpacing: '0.15em', textTransform: 'uppercase', color: '#999', textDecoration: 'none' }}>{t.howItWorks}</a>
        ...
        <main style={{ maxWidth: '640px', margin: '0 auto', padding: '80px 48px' }}>
          ...
          <div style={sectionStyle}>
            ...
            <p style={pStyle}>{t.s1b}</p>
          </div>
          <div style={sectionStyle}>
            <h2 style={h2Style}>{t.s2h}</h2>
            <p style={pStyle}>{t.s2b}</p>
          </div>
          <div style={sectionStyle}>
            <h2 style={h2Style}>{t.s3h}</h2>
            <p style={pStyle}>{t.s3b}</p>
          </div>
          <div style={sectionStyle}>
            <h2 style={h2Style}>{t.s4h}</h2>
            <p style={pStyle}>{t.s4b}</p>
          </div>
        </main>
