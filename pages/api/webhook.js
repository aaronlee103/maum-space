...
// body parser 비활성화 (Stripe 서명 검증에 필요)
...
export default async function handler(req, res) {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    // ── 4. ✅ XSS 방어: 모든 사용자 입력을 HTML 이스케이프 ──
    const safeSenderName = escapeHtml(order.sender_name);
    const safeRecipientName = escapeHtml(order.recipient_name);
    const safeRecipientPhone = escapeHtml(order.recipient_phone);
    const safeRecipientPostcode = escapeHtml(order.recipient_postcode);
    const safeRecipientAddress = escapeHtml(order.recipient_address);
    const safeRecipientDetail = escapeHtml(order.recipient_address_detail || '');
    const safeMessage = escapeHtml(order.message || '');
    const safeSenderEmail = escapeHtml(order.sender_email);

    const itemsHtml = (order.items || [])
      .map(
        (item) =>
          `<tr>
        <td style="padding:10px 8px;border-bottom:1px solid #f0f0f0;color:#333;font-size:14px">${escapeHtml(item.productName || '상품')} × ${parseInt(item.quantity) || 1}</td>
        <td style="padding:10px 8px;border-bottom:1px solid #f0f0f0;text-align:right;color:#555;font-size:14px">$${(parseFloat(item.total) || 0).toFixed(2)}</td>
      </tr>`
      )
      .join('');
    ...
    try {
      await resend.emails.send({
        ...
        to: [order.sender_email], // DB에서 가져온 값 사용
        ...
        html: `
          <div style="font-family:'Helvetica Neue',sans-serif;max-width:560px;margin:0 auto;padding:40px 24px;color:#1a1a1a">
            ...
            <p style="font-size:15px;color:#333;line-height:1.8;margin-bottom:8px">안녕하세요 <strong>${safeSenderName}</strong>님,</p>
            ...
            <p style="font-size:14px;color:#555;line-height:1.9">
              <strong>받는 분:</strong> ${safeRecipientName}<br>
              <strong>연락처:</strong> ${safeRecipientPhone}<br>
              <strong>주소:</strong> [${safeRecipientPostcode}] ${safeRecipientAddress} ${safeRecipientDetail}<br>
              ${safeMessage ? `<strong>메시지:</strong> ${safeMessage}` : ''}
            </p>
            <hr style="border:none;border-top:1px solid #eee;margin:32px 0">
    ...
    try {
      const itemsText = (order.items || [])
        .map((item) => `• ${escapeHtml(item.productName)} × ${parseInt(item.quantity) || 1} — $${(parseFloat(item.total) || 0).toFixed(2)}`)
        .join('<br>');

      await resend.emails.send({
        from: 'Maum Orders <onboarding@resend.dev>',
        to: [process.env.ADMIN_EMAIL || 'aaronlee103@gmail.com'],
        subject: `[새 주문] ${safeSenderName} — $${(order.total_usd || 0).toFixed(2)}`,
        html: `
          <div style="font-family:'Helvetica Neue',sans-serif;max-width:560px;margin:0 auto;padding:40px 24px;color:#1a1a1a">
            <h1 style="font-size:16px;font-weight:500;margin-bottom:4px">새 주문이 들어왔습니다</h1>
            <p style="color:#aaa;font-size:12px;margin-bottom:32px">Order ID: <code>${escapeHtml(String(order.id))}</code></p>
            <table style="width:100%;border-collapse:collapse;font-size:14px;margin-bottom:24px">
              <tr style="background:#f9f9f9">
                <td style="padding:10px 12px;color:#999;width:120px">주문자</td>
                <td style="padding:10px 12px">${safeSenderName} (${safeSenderEmail})</td>
              </tr>
              <tr>
              ...
              <tr style="background:#f9f9f9">
                ...
                <td style="padding:10px 12px">${safeRecipientName} / ${safeRecipientPhone}</td>
              </tr>
              <tr>
                <td style="padding:10px 12px;color:#999">배송지</td>
                <td style="padding:10px 12px">[${safeRecipientPostcode}] ${safeRecipientAddress} ${safeRecipientDetail}</td>
              </tr>
              ${safeMessage ? `<tr style="background:#f9f9f9"><td style="padding:10px 12px;color:#999">메시지</td><td style="padding:10px 12px">${safeMessage}</td></tr>` : ''}
            </table>
